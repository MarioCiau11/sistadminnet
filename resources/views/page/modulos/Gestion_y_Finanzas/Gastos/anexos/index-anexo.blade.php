    ...
    <div class="mainpanel">
        <div class="pageheader">
            <div class="media display-space-between">
                ...
                <h3 class="text-black text-center">{{ $movimiento->expenses_movement }} :
                    <strong>#{{ $movimiento->expenses_movementID }} </strong>
                </h3>
        ...
        <div class="contentpanel">
            <div class="row row-stat">
                            ...
                            <div class="col-sm-12">
                                <div class="row media-manager">
                                    @foreach ($anexosFiles as $anexosFile)
                                        ...
                                        $FileArray = explode('/', $anexosFile['expensesFiles_file']);
                                        $longitudFile = count($FileArray);
                                        $file = $FileArray[$longitudFile - 1];
                                        $quitamosDoblesDiagonales = str_replace(['//', '///', '////'], '/', 'archivo/' . $anexosFile->expensesFiles_path);
                                        ?>
                                                ...
                                                <div class="btn-group fm-group" style="display: block;">
                                                    ...
                                                    <ul class="dropdown-menu fm-menu pull-right" role="menu">
                                                        ...
                                                        <li><a class="eliminar-file" style="cursor: pointer">
                                                                {!! Form::open([
                                                                    'route' => ['modulo.gastos.anexos.delete', 'id' => $anexosFile->expensesFiles_id],
                                                                    'method' => 'DELETE',
                                                                ]) !!}
                ...
                {!! Form::open([
                    'route' => ['modulo.gastos.anexos.store', 'id' => $id],
                    'method' => 'POST',
                    'class' => 'dropzone',
                    'id' => 'myDropzone',
                        ...
                        init: function() {
                            this.on("complete", function(file) {
                                if (this.getUploadingFiles().length === 0 && this.getQueuedFiles()
                                    .length === 0) {
                                    mostrarMensaje('!Listo!', 'Se ha guardado los archivo correctamente',
                                        'success');
                                }
                            });
