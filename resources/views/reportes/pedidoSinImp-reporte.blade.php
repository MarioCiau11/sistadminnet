...
<head>
    ...
    <title>NOTA DE VENTA SIN IMPUESTOS</title>
        ...
        @foreach ($articulos_venta as $key => $articulo)
            <tr>
                ...
                @if(($articulo->salesDetails_id == $kit->procKit_articleIDReference))
                    <p style="margin-left: 20px;">{{ number_format($kit->procKit_cantidad, 2).'-'.$kit->procKit_articleDesp}}</p>
                @endif
            @endforeach
            @else
            ...
            @endif
                ...
                <td style="text-align: right">${{ number_format(($articulo->salesDetails_unitCost * 1.16), 2) }}</td>
                <?php
    ...
    <table class="costos-desglosados">
        ...
        <tr>
            ...
            <td style="border-top: 1px solid black">
                <p class="numeros-reportes" style="text-align: right">$ {{ number_format(($totales-$descuento), 2) }}</p>
            </td>
        </tr>
